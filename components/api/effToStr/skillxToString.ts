...
function triggerToString(s: string): string | null {
  ...
  switch (parts[1]) {
    ...
    case 'more_than_character_count': {
      // @ts-expect-error Unconstricted key
      return `${CharacterChineseNameList['char-' + parts[2]]}在编组中达到${
        parts[3]
      }人时`
    }
...
function effectToString(
  ...
  switch (e.typ) {
    ...
    case 'score_get_by_trigger': {
      return `取得${e.lvl / 10}%得分（当${TRIGGER_TYPE_NAME[e.cond.typ]}达到${
        e.cond.len
      }时为${e.condLvl / 10}%）`
    }
    ...
    case 'stamina_consumption': {
      return `${n}${skillEffectTypeNames[e.typ].replace(/（.*）$/, '')}${e.lvl}`
    }
...
function targetToString(t: TargetChart | TargetCharacterWithCount): string {
  switch (t.typ) {
    ...
    case 'status': {
      return `${TARGET_SPECIAL[t.typ]}[${skillEffectTypeNames[t.typ2]}]时${
        t.cnt
      }人`
    }
  }
